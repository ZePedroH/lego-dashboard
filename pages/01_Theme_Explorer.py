...
if 'df' not in st.session_state:
    st.session_state['df'] = pd.read_csv('https://miles-become-a-data-scientist.s3.us-east-2.amazonaws.com/J3/M2/df_combined_lego.csv')
...
with st.expander("Theme Explorer Sunburst", expanded=True):
    ...
    with st.spinner("Loading..."):
        ...
        df_sunburst['nbr'] = 1
        df_sunburst = df_sunburst.dropna()
        df_sunburst = df_sunburst[df_sunburst['parent_theme_name'] == chosen_theme]

        #👇 Paste the code created in activity 3.1 to create the visualization of the df_sunburst DataFrame
        fig_sunburst = px.sunburst(df_sunburst, path=['parent_theme_name', 'theme_name', 'set_name'], values='nbr', height=800,
