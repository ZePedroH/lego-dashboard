...
filt_n_themes = st.slider('Select the Top N Master Themes', 1, 20, 10)
...
with st.expander(f"New Sets of the Top {filt_n_themes} Master Themes",expanded=True): # Replace ... by the correct method. 🛑Remember that everything contained on the expander needs to be idented.
    ...
    #Step 1 # Group the data by parent theme name and count the number of unique sets
    df_themes_to_parts = df_filtered.groupby('parent_theme_name')['set_name'].nunique().reset_index().rename(columns={'set_name': 'nbr_sets'})
    #Step 2 # Sort the themes by the number of sets in descending order and select the top N themes
    ...
    fig_parent_theme.update_layout(title=f'Number of Distinct Lego Sets per Parent Theme (Top {filt_n_themes})',
                  xaxis_title='Number of Sets', yaxis_title='Parent Theme')
...
with st.expander(f"New Sets of the Top {filt_n_themes} Master Themes", expanded=True):
    ...
    #Step 3
    sets_per_year_master_theme_top = sets_per_year_master_theme[sets_per_year_master_theme['parent_theme_name'].isin(sets_per_parent_theme.parent_theme_name.tolist())].sort_values('parent_theme_name')

    #Step 4
    sets_per_year_master_theme_top['colors'] = sets_per_year_master_theme_top['parent_theme_name'].map(dict_colors)

    #Step 5
    sets_per_year_master_theme_remainder = sets_per_year_master_theme[~sets_per_year_master_theme['parent_theme_name'].isin(sets_per_parent_theme.index)]
    ...
    fig_sets_per_master = px.bar(sets_per_year_master_theme, x='year', y='set_name', color='parent_theme_name', 
             color_discrete_sequence=colors_scaled[:filt_n_themes]+['#808080'])
    fig_sets_per_master.update_layout(title='Number of New Sets per Year for Top Parent Themes',
                  xaxis_title='Year', yaxis_title='Number of New Sets')
...
with st.expander("Largest Lego Set per Year"):
    ...
    #Step 5
    largest_set_year = df_sets_to_theme_group[df_sets_to_theme_group['rank'] == 1].sort_values('year',ascending=False)

    #👇 Paste the code created in activity 1.4 to create the visualization of the largest_set_year DataFrame
    fig_largest_set_year = px.bar(largest_set_year, x='year', y='num_parts', color='parent_theme_name', 
             color_discrete_sequence=colors_scaled, hover_name = 'theme_set')

    fig_largest_set_year.update_layout(title="Largest Set per Year",
              xaxis_title="Year", yaxis_title="Number of New Parts per Set")
